...
#Add public holidays
sa_calendar.addHoliday(Date(8,5,2019))

def prev_business_day(as_date):
	if sa_calendar.isBusinessDay(as_date-1)== True:
		x = as_date - 1
	elif sa_calendar.isBusinessDay(as_date-2)== True:
		x = as_date - 2 
	elif sa_calendar.isBusinessDay(as_date-3)== True:
		x = as_date - 3 
	elif sa_calendar.isBusinessDay(as_date-4)== True:
		x = as_date - 4 
	elif sa_calendar.isBusinessDay(as_date-5)== True:
		x = as_date - 5 
	elif sa_calendar.isBusinessDay(as_date-6)== True:
		x = as_date - 6 
	elif sa_calendar.isBusinessDay(as_date-7)== True:
		x = as_date - 7 
	elif sa_calendar.isBusinessDay(as_date-8)== True:
		x = as_date - 8 
	elif sa_calendar.isBusinessDay(as_date-9)== True:
		x = as_date - 9 
	else:
		x = as_date - 10
	return x

def next_business_day(as_date):
	if sa_calendar.isBusinessDay(as_date+1)== True:
		x = as_date + 1
	elif sa_calendar.isBusinessDay(as_date+2)== True:
		x = as_date + 2 
...
t_1 = t-1
t_2 = t_1 - 1
if  sa_calendar.isBusinessDay(t-1)==False:
	bd_t_1 = prev_business_day(t_1)
else:
	bd_t_1 = t_1
	
t1 = t
if  sa_calendar.isBusinessDay(t)==False:
	bd_t1 = next_business_day(t1)
else:
	bd_t1 = t1

bd_t_2 = prev_business_day(bd_t_1)

mtd = pd.to_datetime("%d-%d-%d" %(bd_t_1.year(),bd_t_1.month(),bd_t_1.dayOfMonth())) + MonthEnd(-1)	
mtd = parser.parse(str(mtd)).date()
mtd = Date(mtd.day,mtd.month,mtd.year)
#mtd = Date(30,4,2019)

if  sa_calendar.isBusinessDay(mtd)==False:
	bd_mtd = prev_business_day(mtd)
else:
	bd_mtd = mtd

ytd = Date(31,12,bd_t_1.year()-1)
if  sa_calendar.isBusinessDay(ytd)==False:
	bd_ytd = prev_business_day(ytd)
else:
	bd_ytd = ytd

# Convert datetime to string date format	
t = "%d-%d-%d" %(t.year(),t.month(),t.dayOfMonth())
t_1 = "%d-%d-%d" %(t_1.year(),t_1.month(),t_1.dayOfMonth())
t_2 = "%d-%d-%d" %(t_2.year(),t_2.month(),t_2.dayOfMonth())
bd_t_1 = "%d-%d-%d" %(bd_t_1.year(),bd_t_1.month(),bd_t_1.dayOfMonth())
bd_t_2 = "%d-%d-%d" %(bd_t_2.year(),bd_t_2.month(),bd_t_2.dayOfMonth())
mtd = "%d-%d-%d" %(mtd.year(),mtd.month(),mtd.dayOfMonth())
bd_mtd = "%d-%d-%d" %(bd_mtd.year(),bd_mtd.month(),bd_mtd.dayOfMonth())
ytd = "%d-%d-%d" %(ytd.year(),ytd.month(),ytd.dayOfMonth())
bd_ytd = "%d-%d-%d" %(bd_ytd.year(),bd_ytd.month(),bd_ytd.dayOfMonth())	
bd_t1 = "%d-%d-%d" %(bd_t1.year(),bd_t1.month(),bd_t1.dayOfMonth())
...
for dates in daterange:
	...
	if sa_calendar.isBusinessDay(datetime_obj)== True:
		bd_datetime_obj = datetime_obj
	else:
		bd_datetime_obj = prev_business_day(datetime_obj)
		
	prev_bd_datetime_obj = prev_business_day(datetime_obj)
	...
	cash_flow_lag_months = 5
	actual_fix_date = '2017-07-01'
	last_cf_profile_date = '2016-07-01'
	...
	df_cpi.rename(columns={'CPI Index': 'CPI', 'Date': 'actual_cpi_date'}, inplace=True)
	...
	df_cpi['ref_value_date'] = df_cpi.value_date - pd.DateOffset(months=cash_flow_lag_months)
	df_cpi['ref_value_date_2'] = df_cpi.value_date - pd.DateOffset(months=4)
	...
	df_cpi = df_cpi[['actual_cpi_date','actual_fix_date','ref_fix_date','ref_fix_m_end_date','value_date','ref_value_date','ref_value_date_2','ref_last_cf_profile_date','CPI']]
	ref_cpi_y_interp = scipy.interpolate.interp1d(excel_date(df_cpi.actual_cpi_date), df_cpi.CPI, fill_value='extrapolate')
	df_cpi['ref_fix_cpi'] = ref_cpi_y_interp(excel_date(df_cpi.ref_fix_date))
	...
	df_cpi['current_cpi_2'] = ref_cpi_y_interp(excel_date(df_cpi.ref_value_date_2))
	df_cpi['last_cf_cpi'] = ref_cpi_y_interp(excel_date(df_cpi.ref_last_cf_profile_date))
	df_cpi['accrual_factor'] = df_cpi.ref_fix_cpi / df_cpi.last_cf_cpi
	df_cpi['implied_cpi'] = df_cpi.accrual_factor - 1
	df_cpi['adjusted_accrual_factor'] = df_cpi.implied_cpi + 1
	df_cpi['multiplier'] = 100/df_cpi['current_cpi_2']
	df_cpi['index_start_value'] = np.where(df_cpi['cpi_base']>100,df_cpi['cpi_base'],df_cpi['fix_m_end_cpi']*df_cpi['multiplier'])
	...
	df = df[(df.PortfolioIDCode == '95933_Active') | (df.PortfolioIDCode == '95933_Pension') | (df.PortfolioIDCode == '95933_ESA')]
	...
	df['index_start_date'] = df_cpi['ref_fix_date'].iloc[-1]
	df_2 = pd.DataFrame(pd.date_range(df_cpi['ref_fix_date'].iloc[-1], df.cash_flow_date.max(), freq='MS'), columns=['index_end_date'])
	df_2 = df_2[df_2.index_end_date.dt.month == df_cpi.ref_fix_date.dt.month.iloc[0]]
	df_2 = pd.concat([df_2]*12)
	df_2 = df_2.sort_values(by=['index_end_date'], ascending=True)
	df_2 = df_2.reset_index()
	df_2 = df_2[['index_end_date']]
	df_2['cash_flow_date'] = pd.DataFrame(pd.date_range(df_cpi['actual_fix_date'].iloc[-1], df.cash_flow_date.max(), freq='MS'), columns=['actual_date']) + pd.DateOffset(months=1) - pd.DateOffset(1)
	...
	df ['index_end_value' ] = np.where(df.check == 'fixed',df['index_start_value'],cpi_base_y_interp(excel_date(df.index_end_date + pd.DateOffset(months=0))))
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\engen\engen_liability_valuation_model_' + datetime_obj + '.xlsx')
