...
for dates in daterange:
    ...
    # df_cpi = pd.read_excel(r'C:\Users\XTW\Documents\py_testing\RMBSi\cpi\CPI Intepolated numbers.xlsm', sheet_name = 'CPI Series')
    ...
    # curve = pd.read_excel(r'C:\Users\XTW\Documents\py_testing\RMBSi\curves\rmb_curves_ldi\rmb_curves_ldi_' + str(pd.to_datetime(bd_datetime_obj).date()) + '.xlsx')
    ...
    # df = pd.read_excel(r'C:\Users\XTW\Documents\py_testing\RMBSi\cashflows\LDICashflows.xlsx')
    df = df[df.PortfolioIDCode == 95936]
    ...
    # Export file
    # writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMBSi\RMBSi_liability_valuation_model_' + datetime_obj + '.xlsx')
    writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMBSi\RMBSi_liability_valuation_model_' + datetime_obj + '.xlsx')
    ...
    # curve = pd.read_excel(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\curves\rmb_curves_ldi\rmb_curves_ldi_' + str(pd.to_datetime(Prev_bd_datetime_obj).date()) + '.xlsx')
    # # curve = pd.read_excel(r'C:\Users\XTW\Documents\py_testing\RMBSi\curves\rmb_curves_ldi\rmb_curves_ldi_' + str(pd.to_datetime(prev_bd_datetime_obj).date()) + '.xlsx')
    # bond_curve = curve[curve['Name'] == 'RMBZERO']
    # bond_curve.rename(columns={'Date': 'TenorDate', 'Rate': 'BondCurveYield'}, inplace=True)
    # bond_curve = bond_curve[['TenorDate','BondCurveYield']]
    # bond_curve['BondCurveYield'] = (1 + bond_curve['BondCurveYield']/2)**2 - 1   # converting comp freq from NACS to NACA
    ...
    # # Export file
    # # writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMBSi\RMBSi_liability_valuation_model_' + datetime_obj + '.xlsx')
    # writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMBSi\RMBSi_liability_valuation_model_' + datetime_obj + '_prev.xlsx')
    # df.to_excel(writer, 'Valuation', index=False)
    # curve.to_excel(writer, 'Curves', index=False)
    # writer.save()
