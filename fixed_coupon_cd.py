...
df = df[df.InstrumentTypeDescription == 'MMI : FIXED COUPON CD']
...
for row in range(len(df)):
	print('fixed_coupon_cd', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'ytm')
...
pieces = []
for row in range(len(df)):
	print('fixed_coupon_cd', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\fixed_coupon_cd.csv',index=False)