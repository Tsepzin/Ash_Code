...
writer = pd.ExcelWriter(r'Z:\Investment Analytics\Python\Attribution' + datetime_obj_start + '_' + datetime_obj_end + '.xlsx')
...
df_jse = pd.read_excel(r'Z:\Investment Analytics\Python\take_on\config\manual_jse_mtm.xlsx')
...
for i in PfCode:
	...
	for dates in daterange:
		...
		# Instrument type
		sql = "SELECT InstrumentCode, InstrumentTypeDescription, MaturityDate FROM Investment.MaitlandASISAInstruments WHERE DataDate = " + " ' " + datetime_obj_end + "'" + " AND InstrumentCurrency = 'ZAR'"
       ...
       # Previous Holdings
		# sql = "SELECT InstrumentCode, PortfolioCode, PortfolioName, BaseCurrency, InstrumentLongName, HoldingsNominal, EffectiveExpoure, AccruedIncome FROM Investment.vwMaitlandHoldings WHERE ValuationDate = " + " '" + str((pd.to_datetime(dates) + pd.DateOffset(days=-1)).date()) + "'" + " AND PortfolioCode = " + "'" + i + "'" +""
        # df = pd.read_sql(sql, db)
		df = pd.read_excel(r'Z:\Investment Analytics\Python\take_on\config\manual_holdings.xlsx')
		df.PortfolioCode = df.PortfolioCode.astype(str)
		df = df[(df.ValuationDate == str((pd.to_datetime(dates) + pd.DateOffset(days=-1)).date())) & (df.PortfolioCode == str(i))]
		df = df.drop(columns=['ValuationDate'])        
		...
		df.rename(columns={'PortfolioCode':'PortfolioIDCode','EffectiveExpoure': 'PrevEffectiveExpoure', 'AccruedIncome': 'PrevAccruedIncome', 'HoldingsNominal': 'PrevHoldingsNominal'}, inplace=True)
		
		# Current Holdings
		# sql = "SELECT InstrumentCode, ValuationDate, PortfolioCode, PortfolioName, BaseCurrency, InstrumentLongName, HoldingsNominal, EffectiveExpoure FROM Investment.vwMaitlandHoldings WHERE ValuationDate = " + " ' " + str(dates.date()) + "'" + " AND PortfolioCode = " + "'" + i + "'" +""
		# df1 = pd.read_sql(sql, db)
		df1 = pd.read_excel(r'Z:\Investment Analytics\Python\take_on\config\manual_holdings.xlsx')
		df1.PortfolioCode = df1.PortfolioCode.astype(str)
		df1 = df1[(df1.ValuationDate == str(dates.date())) & (df1.PortfolioCode == str(i))]
		df1 = df1.drop(columns=['AccruedIncome']) 
		df1 = df1[['InstrumentCode', 'ValuationDate', 'PortfolioCode', 'PortfolioName', 'BaseCurrency', 'InstrumentLongName', 'HoldingsNominal', 'EffectiveExpoure']]
		...
		df1 = df1[['InstrumentCode', 'HoldingsNominal', 'ValuationDate', 'CurAllInPrice']]
		df1.rename(columns={'PortfolioCode':'PortfolioIDCode','HoldingsNominal': 'CurHoldingsNominal'}, inplace=True)
		...
		# df1 = pd.read_sql(sql, db)
		# df1 = pd.read_excel(r'Z:\Investment Analytics\Python\take_on\config\manual_jse_mtm.xlsx')
		df1 = df_jse[(df_jse.Date == str((pd.to_datetime(dates) + pd.DateOffset(days=-1)).date()))]
		...
		# df1 = pd.read_sql(sql, db)
		# df1 = pd.read_excel(r'Z:\Investment Analytics\Python\take_on\config\manual_jse_mtm.xlsx')
		df1 = df_jse[df_jse.Date == str(dates.date())]
