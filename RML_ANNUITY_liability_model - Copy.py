...
for dates in daterange:
	...
	# Parameters
	InvestmentCPISpread_bps = 50
	MedicalCPISpread_bps = 100
	InflationLag_months = 12
	InflationIndexation_months = 4
	last_cf_profile_date = '2019-08-01'
	actual_fix_date = '2019-08-01'
	...
	df = df[(df.PortfolioIDCode == 95924)]
	...
	df_3 = pd.DataFrame(pd.date_range(df_cpi['ref_fix_date'].iloc[-1], df.cash_flow_date.max(), freq='MS'), columns=['actual_date'])
	df_4 = pd.concat([df_2,df_3], axis=1)
	df_4 ['cash_flow_date'] = df_4.actual_date + pd.DateOffset(days=14)
	df_4 = df_4[['cash_flow_date','index_end_date']]
	df = df.merge(df_4, how='left', on=['cash_flow_date'])
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMA_RML\RML_ANNUITY\RML_ANNUITY_liability_valuation_model_' + datetime_obj + '.xlsx')
