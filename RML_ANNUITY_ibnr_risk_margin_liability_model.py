...
for dates in daterange:
	...
	last_cf_profile_date = '2020-09-01'
	...
	df = df[(df.PortfolioIDCode == '95924_ibnr_risk_margin')]
	...
	my_dates = pd.read_excel(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\config\my_dates_ibnr_risk_margin.xlsx')
	df = df.merge(my_dates, how='left', on=['cash_flow_date'])
	df['index_start_value'] = df_cpi['index_start_value'].iloc[0]
	df['index_end_value'] = np.where(df.check == 'Fixed', df.index_start_value, cpi_base_y_interp(excel_date(df.index_end_date)))
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMA_RML\RML_ANNUITY\RML_ANNUITY_ibnr_risk_margin_liability_valuation_model_' + datetime_obj + '.xlsx')
