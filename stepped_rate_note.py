...
df = df[df.InstrumentTypeDescription == 'MMI : STEPPED RATE NOTE']
...
# update bp spreads
df_bp_spread = pd.read_excel(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\mapping_table\mapping_table.xlsx', sheet_name='stepped_rate_note')
...
for row in range(len(df)):
	print('stepped_rate_note', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\stepped_rate_note.csv',index=False)
