...
for dates in daterange:
	...
	MedicalCPISpread_bps = 125
	InflationLag_months = 12
	InflationIndexation_months = 4
	InflationIncluded_medical = '2021-06-30'
	InflationIncluded_annuities = '2017-10-01'
	last_cf_profile_date = '2021-06-30'
	...
	df_cpi['ref_cpi_date'] = df_cpi.actual_cpi_date + pd.DateOffset(months=InflationIndexation_months)
	df_cpi = df_cpi[['actual_cpi_date','ref_cpi_date','value_date','last_cf_profile_date','RefCPI']]
	df_cpi['x_axis'] = excel_date(df_cpi.ref_cpi_date)
	cpi_y_interp = scipy.interpolate.interp1d(df_cpi.x_axis, df_cpi.RefCPI)
	df_cpi['current_cpi'] = cpi_y_interp(excel_date(df_cpi.value_date))
	df_cpi['last_cf_cpi'] = cpi_y_interp(excel_date(df_cpi.last_cf_profile_date))
	df_cpi['accrual_factor'] = df_cpi.current_cpi / df_cpi.last_cf_cpi
	df_cpi['implied_cpi'] = df_cpi.accrual_factor ** (365/(df_cpi.value_date.dt.date - df_cpi.last_cf_profile_date.dt.date).dt.days) - 1
	df_cpi['adjusted_accrual_factor'] = (1+df_cpi.implied_cpi+MedicalCPISpread_bps/10000) ** ((df_cpi.value_date.dt.date - df_cpi.last_cf_profile_date.dt.date).dt.days/365)
	...
	df = df[(df.PortfolioIDCode == '95027_expenses')]
	df = df[(df.cash_flow_date > datetime_obj)]
	df['adjustedRCF'] = df_cpi['adjusted_accrual_factor'].iloc[-1] * df.OriginalRCF
	...
	df['nominal_cf_base'] = np.where(df.cash_flow_date == df.CurrentDate, 0,df.adjustedRCF*(1+df['be_rate_base'])**df.t)
	...
	df['nominal_cf_base_today'] = np.where(df.cash_flow_date == df.CurrentDate,df.adjustedRCF*(1+df['be_rate_base'])**df.t,0)
	...
	# Nominal XIRR 
	df_xirr = df[(df.cash_flow_date > datetime_obj)]
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMA_RML\RMA_LLM\RMA_LLM_expenses_liability_valuation_model_' + datetime_obj + '.xlsx')
