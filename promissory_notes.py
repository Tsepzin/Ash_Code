...
df = df[df.InstrumentTypeDescription == 'MMI : PROMISSORY NOTES']
...
for row in range(len(df)):
	print('promissory_notes', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'ytm')
...
pieces = []
for row in range(len(df)):
	print('promissory_notes', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\promissory_notes.csv',index=False)