...
df = df[df.InstrumentTypeDescription == 'MMI : TREASURY BILL']
...
for row in range(len(df)):
	print('t_bill', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'ytm')
...
pieces = []
for row in range(len(df)):
	print('t_bill', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\t_bill.csv',index=False)