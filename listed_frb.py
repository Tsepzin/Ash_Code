...
df = df[df.InstrumentTypeDescription == 'BOND : FIXED RATE BOND']
...
for row in range(len(df)):
	print('listed_frb', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'ytm')
...
pieces = []
for row in range(len(df)):
	print('listed_frb', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\listed_frb.csv',index=False)