...
df = df[df.InstrumentTypeDescription == 'BOND : ZERO COUPON BOND']
...
for row in range(len(df)):
	print('zero_coupon', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'ytm')
...
pieces = []
for row in range(len(df)):
	print('zero_coupon', df.iloc[row,df.columns.get_loc('InstrumentCode')], 'price')
...
df.to_csv(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\instrument_data\input\zero_coupon.csv',index=False)