...
sql = "SELECT InstrumentCode, ((MarketValue - AccruedIncome)/NULLIF(HoldingsNominal, 0)*100) AS CleanPrice, ((MarketValue)/NULLIF(HoldingsNominal, 0)*100) AS AllInPrice FROM Investment.vwMaitlandHoldings WHERE ValuationDate = " + " ' " + datetime_obj_start + "'" + ""
...
for i in PfCode:
    pieces = []
    sql = "SELECT InstrumentCode, PortfolioCode, PortfolioName, ValuationDate, BaseCurrency, InstrumentLongName, HoldingsNominal,  BaseEffectiveExposure ,(BaseEffectiveExposure/NULLIF(TotalMarketValue,0)) AS Weight1,BaseMarketValue,(BaseMarketValue/NULLIF(TotalMarketValue,0)) AS Weight2, ((BaseMarketValue - AccruedIncome)/NULLIF(HoldingsNominal, 0)*100) AS CleanPrice FROM Investment.vwMaitlandHoldings WHERE ValuationDate = " + " '" + datetime_obj_start + "'" + " AND PortfolioCode = " + "'" + str(i) + "'" +""
    ...
    df['ValuationDate']= df.apply(lambda row :row['ValuationDate'].strftime('%Y-%m-%d'), axis=1, result_type='reduce')
    df.to_excel(writer, str(i), index=False)
   
writer.save()

sql = "SELECT BondCode, TradeDate, Maturity, AllInPrice, ModifiedDuration, AccruedInterest, Coupon, MTM FROM Investment.vwDailyBondMTMGap WHERE TradeDate = " + " ' " + datetime_obj_start + "'" + " "
df_mtm = pd.read_sql(sql, db)
df_mtm = df_mtm[df_mtm.ModifiedDuration != 0]
df_mtm['Freq'] = 2.0
df_mtm['CalDur'] = 0.0
df_mtm['Diff'] = 0.0
df_static1 = df_static[['InstrumentCode','InstrumentTypeDescription']]
df_static1.rename(columns={'InstrumentCode': 'BondCode'}, inplace=True)
df_mtm = df_mtm.merge(df_static1,  how='left', on=['BondCode'])
df_mtm = df_mtm[df_mtm['InstrumentTypeDescription'].notnull()]
