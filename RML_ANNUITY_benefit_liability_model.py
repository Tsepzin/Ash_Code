...
for dates in daterange:
	...
	df_jse_zero_curve['be_base'] = (1+df_jse_zero_curve['bond_curve_base'])/(1+df_jse_zero_curve['real_curve_base'])-1 + 150/10000
	# Linking functions
	...
	MedicalCPISpread_bps = 150
	InflationLag_months = 12
	InflationIndexation_months = 4
	last_cf_profile_date = '2020-12-01'
	actual_fix_date = '2021-04-01'
	#ref_fix_date = '2018-04-01'
	...
	df_cpi['ref_fix_date'] = df_cpi.actual_fix_date - pd.DateOffset(months=InflationIndexation_months)
	...
	df_cpi['ref_last_cf_profile_date'] = df_cpi.last_cf_profile_date - pd.DateOffset(months=InflationIndexation_months)
	df_cpi = df_cpi[['actual_cpi_date','actual_fix_date','ref_fix_date','value_date','ref_value_date','ref_last_cf_profile_date','RefCPI']]
	...
	df_cpi['cpi_base'] = cpi_base_y_interp(excel_date(df_cpi.ref_last_cf_profile_date))
	df_cpi['multiplier'] = 100/df_cpi['current_cpi']
	df_cpi['index_start_value'] = df_cpi['fix_cpi']*df_cpi['multiplier']
	# df_cpi['index_start_value'] = df_cpi['cpi_base']
	...
	df = df[(df.PortfolioIDCode == '95924_benefit')]
	...
	my_dates = pd.read_excel(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\config\my_dates_benefit.xlsx')
	df = df.merge(my_dates, how='left', on=['cash_flow_date'])
	df['index_start_value'] = cpi_base_y_interp(excel_date(df.index_start_date))
	df['index_end_value'] = cpi_base_y_interp(excel_date(df.index_end_date))
	df['accrual_factor_base'] = df.index_end_value/df.index_start_value
	df['nominal_cf_base'] = np.where(df.cash_flow_date == df.CurrentDate, 0,df.OriginalRCF*df.accrual_factor_base*df.mar22_accrual)
	...
	df['nominal_cf_base_today'] = np.where(df.cash_flow_date == df.CurrentDate,df.OriginalRCF*df.accrual_factor_base,0)
	df['nominal_cf_base_today'] = df['nominal_cf_base_today'].iloc[0]
	df = df[(df.cash_flow_date > datetime_obj)]
	df['duration_base'] = df['t'] * df['npv_base']
	df['total_duration_base'] = df['duration_base'].sum() / df['total_npv_base']
	df['convexity_base'] = (df['t'] + df['t']**2) * df['npv_base']
	df['total_convexity_base'] = df['convexity_base'].sum() / df['total_npv_base']
	df['real_duration_base'] = df['t'] * df['real_npv_base']
	df['total_real_duration_base'] = df['real_duration_base'].sum() / df['total_real_npv_base']
	df['real_convexity_base'] = (df['t'] + df['t']**2) * df['real_npv_base']
	df['total_real_convexity_base'] = df['real_convexity_base'].sum() / df['total_real_npv_base']
	df['weighted_convexity_base'] = (df['total_npv_base'] * df['total_convexity_base'] + df['total_real_npv_base'] * df['total_real_convexity_base']) / (df['total_npv_base'] + df['total_real_npv_base'])
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\RMA_RML\RML_ANNUITY\RML_ANNUITY_benefit_liability_valuation_model_' + datetime_obj + '.xlsx')
