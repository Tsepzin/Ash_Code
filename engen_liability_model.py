...
for dates in daterange:
	...
	actual_fix_date = '2018-07-01'
	last_cf_profile_date = '2017-07-01'
	...
	# df ['index_end_value' ] = np.where(df.check == 'fixed',df['index_start_value'],cpi_base_y_interp(excel_date(df.index_end_date + pd.DateOffset(months=InflationIndexation_months)))) # 29/08/2019
	...
	# Export file
	writer = pd.ExcelWriter(r'\\rmb-vpr-file02\Ashburton\Support Services\Risk Management\Investment Analytics\Python\take_on\reports\liability_models\engen\engen_liability_valuation_model_' + datetime_obj + '_prev.xlsx')
	df_info.to_excel(writer, 'Info')
	df.to_excel(writer, 'Valuation', index=False)
	df_jse_zero_curve.to_excel(writer, 'JSE Yield Curves', index=False)
	writer.save()
